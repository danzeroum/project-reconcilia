...
**Este repositório é `derived`.** Ele nasceu de `/adotar` rodado no molde e governa exatamente um
alvo — o declarado em `project.yaml:target`, ancorado por SHA em `target.lock` e materializado em
`workspace/target/` por `python ci/bootstrap.py` (efêmero, fora do versionamento). Não tem código
de negócio próprio, e isso é deliberado: o `CP-000` removeu o negócio de exemplo que o molde
carrega, porque metadado que descreve um sistema que o repositório não governa é exatamente a
doença que esta arquitetura existe para impedir.

O que ele ainda não tem é o metadado do alvo. Código e teste do alvo aparecem como **órfãos** em
`python ci/validate_all.py` até que a ingestão os reivindique — é o estado que
[`docs/COMO-ADOTAR.md`](docs/COMO-ADOTAR.md) descreve como "nasce vermelho por construção; ficar
verde é o trabalho". O próximo passo é `/ingerir`.
